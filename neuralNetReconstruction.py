...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        gt_sdf_tensor = data['gt_sdf'].to(device) # ground truth values defined in utils.py

        # Calculate the loss
        clamping_value = 0.1 
        l1_clamped = abs(clamp(pred_sdf_tensor,-clamping_value,clamping_value) - clamp(gt_sdf_tensor,-clamping_value,clamping_value)) 
        l1_loss = l1_clamped.sum()

        # Perform backpropagation on the loss
        l1_loss.backward()  

        loss_sum += l1_loss.item()
        loss_count += xyz_tensor.shape[0]
