...
def naiveReconstruction(points, normals, X, Y, Z):
    ...
    distances, indexes = tree.query(Q, k=1) #  k -> number of nearest neighbors to return

    squeezed_indexes = indexes.squeeze() # indexes were (884736, 1) so we squeezed them to (884736, )

    partial_IF = Q - points[squeezed_indexes]

    IF_unshaped = np.sum(normals[squeezed_indexes] * partial_IF, axis=1) # dot product row by row
    # more information available: https://stackoverflow.com/questions/15616742/vectorized-way-of-calculating-row-wise-dot-product-two-matrices-with-scipy

    IF = IF_unshaped.reshape(X.shape)
